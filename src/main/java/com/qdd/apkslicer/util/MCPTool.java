...
public class MCPTool {
    ...
    private final static Logger logger = LoggerFactory.getLogger(MCPTool.class);
    /**
     * 数据结构体的签名标记
     */
...
//	private static final String CHARSET_NAME = "GBK";
     ...
     * @throws Exception
     */
    public static void write(File path, String content, String password)
            throws Exception {
     ...
     * @throws Exception
     */
    private static void write(File path, byte[] content, String password)
            throws Exception {
...
//					"Zip comment is exists, Repeated write is not recommended.");
//		}
        ...
        raf.write(short2Stream((short) data.length)); // 重新写入comment长度，注意Android
        // apk文件使用的是ByteOrder.LITTLE_ENDIAN（小端序）；
        raf.write(data);
        raf.close();
    }
    ...
    private static byte[] read(File path, String password) throws Exception {
        ...
        if (VERSION_1_1.equals(version)) {
            ...
            index -= bytes.length;
            readFully(raf, index, bytes); // 读取内容长度；
            long lengthContent = stream2Short(bytes, 0);
            bytesContent = new byte[(int) lengthContent];
     ...
     * @throws IOException
     */
    private static void readFully(RandomAccessFile raf, long index,
    ...
    public static String readContent(File path, String password) {
        try {
            byte[] bytes = read(path, password);
            if (bytes == null) {
                return null;
            }
            return new String(bytes, CHARSET_NAME);
        } catch (Exception ignore) {
            logger.debug("Failed to read content from MCPTool", ignore);
        }
        return null;
    }
     ...
     *
     * @param context
     *            Android中的android.content.Context对象
     * @param mcptoolPassword
     *            mcptool解密密钥
     * @param defValue
     *            读取不到时用该值作为默认值
     * @return
     */
    public static String getChannelId(Object context, String mcptoolPassword,
                                      String defValue) {
        String content = MCPTool.readContent(new File(
                getPackageCodePath(context)), mcptoolPassword);
    ...
    private static String getPackageCodePath(Object context) {
        try {
            return (String) context.getClass().getMethod("getPackageCodePath")
                    .invoke(context);
        } catch (Exception ignore) {
        }
        return null;
     ...
     * @throws Exception
     */
    private static byte[] encrypt(String password, byte[] content)
            throws Exception {
     ...
     * @throws Exception
     */
    private static byte[] decrypt(String password, byte[] content)
            throws Exception {
     ...
     * @throws Exception
     */
    private static byte[] cipher(int cipherMode, String password, byte[] content)
            throws Exception {
    ...
    public static boolean nioTransferCopyLarge(File source, File target)
            throws IOException {
        FileChannel in = null;
        FileChannel out = null;
        FileInputStream inStream = null;
     ...
     * String(decrypt(password, contentE.getBytes(CHARSET_NAME)), CHARSET_NAME);
     ...
     *
     */
    public static void pkgApk(String apkPath, String outPath,
                              String channelCode, String password) throws Exception {
        ...
        if (path != null) {
            ...
            if (channelCode != null && !"".equals(channelCode)) {
                System.out.println("contents: " + channelCode);
            }
            if (channelCode == null || "".equals(channelCode)) { // 读取数据；
                System.out.println("content: "
                        + readContent(path, null));
            } else { // 写入数据；
                String fileName = path.getName();
                int dot = fileName.lastIndexOf(".");
                String prefix = fileName.substring(0, dot);
                String suffix = fileName.substring(dot);
                File target = new File(outdir, prefix + "_"
                        + channelCode + suffix);
                if (nioTransferCopyLarge(path, target)) {
                    write(target, channelCode, password);
                }
            }
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        args = "-path ./test.apk -outdir ./output/ -contents xiaomi;360; -password YOUR_PASSWORD".split(" ");
//      args = "-version".split(" ");
//      args = "-path ./test.apk -password YOUR_PASSWORD".split(" ");
        ...
        String help = "用法：java -jar MCPTool.jar [" + cmdPath + "] [arg0] [" + cmdOutdir + "] [arg1] [" + cmdContents + "] [arg2] [" + cmdPassword + "] [arg3]"
                ...
                + "\n例如："
                + "\n写入：java -jar MCPTool.jar -path ./test.apk -outdir ./ -contents googleplay;m360; -password YOUR_PASSWORD"
                + "\n读取：java -jar MCPTool.jar -path ./test.apk -password YOUR_PASSWORD";
